...
const Workspace = () => {
  ...
  useEffect(() => {
    ...
    const handleReceive = (newMsg) => {
      ...
      const isForCurrentChannel = currentDest?.isChannel && incomingChannelId === currentDest._id;
      const isForCurrentDM = !currentDest?.isChannel && (incomingSenderId === currentDest?._id || incomingReceiverId === currentDest?._id);
      const isFromMe = incomingSenderId === user._id;
      ...
      if (isForCurrentChannel || (isForCurrentDM && !isFromMe) || isFromMe) {
        ...
        setUnreadCounts(prev => ({
          ...prev,
          [idToIncrement]: (prev[idToIncrement] || 0) + 1
        }));
      }
    };
    ...
    const handleUpdate = (updatedMsg) => {
      ...
      if (activeThreadRef.current && activeThreadRef.current._id === updatedMsg._id) setActiveThread(updatedMsg);
    };
  ...
  const handleSubmit = async (text, fileUrl = "") => {
    ...
    try {
      ...
      if (activeThread) setThreadMessages((prev) => [...prev, res.data]);
  ...
  const handleDeleteMessage = async (msgId, type) => {
    try {
      ...
      if (type === "everyone") socket.emit("deleteMessage", res.data);
  ...
  const handleFileUpload = async (e) => {
    ...
    data.append("file", file); data.append("upload_preset", uploadPreset); data.append("cloud_name", cloudName);          
    try {
      const res = await fetch(`https://api.cloudinary.com/v1_1/${cloudName}/image/upload`, { method: "POST", body: data });
      const cloudData = await res.json();
      if (cloudData.secure_url) handleSubmit("Sent an attachment 📎", cloudData.secure_url);
      else alert("Cloudinary connection rejected.");
        ...
        <div className={`p-3 rounded-2xl text-sm shadow-xs relative ${isMe && !isThreadView ? "bg-[#1164A3] text-white rounded-tr-none" : "bg-white text-gray-800 border border-gray-200 rounded-tl-none"}`}>
          ...
          {msg.fileUrl && !msg.isDeletedForEveryone && ( 
            <div className="mt-2 rounded-lg overflow-hidden border bg-gray-100 p-1 flex flex-col gap-1 max-w-[240px]">
              ...
              <a href={getDownloadUrl(msg.fileUrl)} download className="bg-white text-[#1164A3] hover:bg-gray-50 text-[11px] font-bold py-1 px-2 rounded border text-center block shadow-2xs transition">📥 Download</a>
            </div>
          )}
        </div>
        {msg.reactions && msg.reactions.length > 0 && !msg.isDeletedForEveryone && ( 
          <div className={`flex flex-wrap gap-1 mt-1 ${isMe && !isThreadView ? "justify-end" : "justify-start"}`}>
            {msg.reactions.map(r => {
              ...
              return (
                <button key={r.emoji} onClick={() => handleReaction(msg._id, r.emoji)} className={`text-[11px] px-1.5 py-0.5 rounded-full border flex items-center gap-1 transition ${hasReacted ? 'bg-blue-100 border-blue-300' : 'bg-white border-gray-300 hover:bg-gray-100'}`}>
                  <span>{r.emoji}</span><span className={hasReacted ? 'text-blue-700 font-bold' : 'text-gray-600'}>{r.users.length}</span>
                </button>
              )
            })}
          </div>
        ...
        {!msg.isDeletedForEveryone && ( 
          <div className={`flex gap-3 text-[11px] font-semibold mt-1 px-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition absolute ${isMe && !isThreadView ? "md:-left-36 md:top-4 right-0 -bottom-8 md:-bottom-auto" : "md:-right-36 md:top-4 left-0 -bottom-8 md:-bottom-auto"} bg-white border shadow-sm rounded-lg p-1 z-10`}>
            <div className="relative">
              <button onClick={() => setShowPickerFor(showPickerFor === msg._id ? null : msg._id)} className="hover:bg-gray-100 p-1 rounded">😀</button>
              {showPickerFor === msg._id && (
                <div className="absolute top-8 md:top-auto md:bottom-8 -left-4 bg-white border shadow-xl rounded-lg p-2 flex gap-1 z-50 w-max">
                  {EMOJIS.map(e => <button key={e} onClick={() => handleReaction(msg._id, e)} className="hover:bg-gray-100 text-lg p-1 rounded transition hover:scale-125">{e}</button>)}
                </div>
              )}
            </div>
    ...
    <div className="flex h-screen bg-gray-50 text-gray-800 font-sans overflow-hidden relative">
      ...
      <div className={`w-64 bg-[#3F0E40] text-white flex flex-col shadow-lg z-40 absolute h-full transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-4 font-bold text-xl border-b border-white/10 flex justify-between items-center">
          <div className="flex items-center gap-3">
             <span>Workspace</span>
             <button onClick={() => setIsSettingsOpen(true)} className="text-gray-300 hover:text-white transition"><Settings size={18} /></button>
          </div>
          <button className="md:hidden text-gray-300 hover:text-white" onClick={() => setIsSidebarOpen(false)}><X size={20} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 select-none">
          <div className="flex justify-between items-center mb-3 mt-2 text-gray-300">
            ...
            <button onClick={() => setIsModalOpen(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs px-2 py-1 rounded shadow-sm">+ Create</button>
          </div>
          <div className="mb-6 border-b border-white/10 pb-4 flex flex-col gap-1">
            {channels.map(c => (
              <div key={c._id} onClick={() => { setActiveDestination({ ...c, isChannel: true }); setActiveThread(null); setIsSidebarOpen(false); }} className={`cursor-pointer px-3 py-2 rounded-md transition font-medium text-sm flex items-center gap-1 ${activeDestination?.isChannel && activeDestination?._id === c._id ? "bg-[#1164A3] text-white shadow" : "hover:bg-white/10 text-gray-300"}`}>
                <span className="text-gray-400 flex-1"># {c.name}</span>
                {unreadCounts[c._id] > 0 && <span className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{unreadCounts[c._id]}</span>}
              </div>
            ))}
          </div>

          <div className="mb-3 text-gray-300"><h3 className="font-semibold text-xs uppercase tracking-wider">Direct Messages</h3></div>
          <div className="flex flex-col gap-1">
            {users.map(u => {
              const isUserOnline = onlineUserIds.includes(u._id);
              return (
                <div key={u._id} onClick={() => { setActiveDestination({ ...u, isChannel: false }); setActiveThread(null); setIsSidebarOpen(false); }} className={`cursor-pointer px-3 py-2 rounded-md transition flex items-center gap-2 text-sm ${!activeDestination?.isChannel && activeDestination?._id === u._id ? "bg-[#1164A3] text-white shadow" : "hover:bg-white/10 text-gray-300"}`}>
                  <div className="relative">
                    {u.profilePic ? ( <img src={u.profilePic} alt={u.username} className="w-5 h-5 rounded-full object-cover shadow-sm" /> ) : ( <div className="w-5 h-5 rounded-full bg-blue-500 flex justify-center items-center text-[10px] font-bold text-white shadow-sm">{u.username[0].toUpperCase()}</div> )}
                    {isUserOnline && <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-green-500 border-2 border-[#3F0E40] rounded-full"></span>}
                  </div>
                  <span className="flex-1 truncate">{u.username}</span>
                  {unreadCounts[u._id] > 0 && <span className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{unreadCounts[u._id]}</span>}
                </div>
              );
            })}
          </div>
      ...
      <div className="flex-1 flex flex-col bg-white relative z-10 w-full overflow-hidden">
        <div className="p-4 border-b shadow-sm flex justify-between items-center bg-white z-10">
          <div className="flex items-center gap-3">
            <button className="md:hidden text-gray-600 hover:text-gray-900" onClick={() => setIsSidebarOpen(true)}><Menu size={24} /></button>
            {activeDestination ? (
              <>
                {activeDestination.isChannel ? (
                  <div className="w-10 h-10 rounded bg-gray-100 flex justify-center items-center text-gray-500 text-xl font-bold shadow-sm">#</div>
                ) : activeDestination.profilePic ? (
                  <img src={activeDestination.profilePic} alt={activeDestination.username} className="w-10 h-10 rounded object-cover shadow-sm" />
                ) : (
                  <div className="w-10 h-10 rounded bg-blue-500 flex justify-center items-center text-lg text-white font-bold shadow-sm">{activeDestination.username[0].toUpperCase()}</div>
                )}
                <div className="flex flex-col">
                  <span className="font-bold text-lg truncate max-w-[150px] md:max-w-none">{activeDestination.name || activeDestination.username}</span>
                  {!activeDestination.isChannel && (
            ...
            ) : <span className="text-gray-400 font-normal">Select a conversation</span>}
          </div>
          
          {activeDestination && <button onClick={handleClearChat} className="text-xs text-red-500 hover:bg-red-50 px-2 md:px-3 py-1.5 rounded border border-red-200 transition font-medium whitespace-nowrap">🧹 <span className="hidden md:inline">Clear Chat</span></button>}
        </div>
      ...
      {activeThread && (
        <div className="fixed right-0 h-full w-full sm:w-80 bg-gray-50 shadow-2xl flex flex-col border-l z-50 md:relative md:z-30 transition-all duration-300">
          <div className="p-4 border-b font-bold flex justify-between items-center bg-white shadow-sm z-10"><span className="text-sm font-bold">Thread</span><button onClick={() => setActiveThread(null)} className="text-gray-400 hover:text-gray-700 text-sm p-2">✖</button></div>
          <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-4"><div className="pb-3 border-b border-gray-200"><MessageBubble msg={activeThread} isThreadView={true} /></div>{threadMessages.map(reply => <MessageBubble key={reply._id} msg={reply} isThreadView={true} />)}<div ref={threadScrollRef}></div></div>
          <div className="p-4 border-t bg-white z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]"><input type="text" placeholder="Reply..." onKeyDown={(e) => { if (e.key === "Enter" && e.target.value.trim()) { handleSubmit(e.target.value); e.target.value = ""; } }} className="w-full p-2 text-sm border rounded-lg outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500 shadow-sm" /></div>
        </div>
      )}
